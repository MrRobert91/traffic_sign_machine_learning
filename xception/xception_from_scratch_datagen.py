...
fichero_log = (code_path +'Xception_load_data.log')

fichero_log_tb = (code_path +'Xception_load_data.log')
...
print ("[STATUS] --------Xception load_data - start time - {}".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M")))
logging.info(" ---------Xception load_data - start time - {}".format(datetime.datetime.now().strftime("%Y-%m-%d %H:%M")))
...
filepath = code_path + "xception_from_scratch" + "-epochs" + str(epochs) + ".h5"
...
model_filename= ("xception_load_data_epochs%s_test_acc_%.2f%%_%s.h5" % (epochs,test_accuracy[1] * 100, today_date))
...
print("Fin de la prueba Xception load_data")
logging.info("-----------Fin de la prueba con Xception load_data-----------")
